 ...
 * enabled.
 *
 * TODO - incorporate timeout better
 * TODO - pipe contents to temp file before sending to destination
 * 
 */
...
class HTTP {
    ...
    public function execute () {
        ...
        if (count($body)) {
            $body = implode("", $body);
            $len = strlen($body);
            if (strtolower($op["enctype"]) === self::ENC_MULTI) {
        ...
        /*
        if ($op["proxy"]) {
            stream_context_set_option($ctx, "http", "proxy", "tcp://ipaddress:port");
        }
        */
        ...
        while (!feof($sock) && !$timeout) {
            ...
            if ($mode == "chunked") {
                ...
                $read = strlen($line);
                while ($sock && $read < $length && !feof($sock)) {
